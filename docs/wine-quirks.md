...
| **MSI packages** | `wine package.msi` fails — an MSI is a Windows Installer *database*, not a program, so Wine has nothing to exec | Route through Wine's built-in `msiexec /i`, passing the **bare filename** with cwd set to the package's folder (no unix→Windows path translation; sibling `.cab` files stay reachable). Classification is by content, defaulting to "run it" so an unparseable header can't break a working install. Exit 3010 = installed-but-wants-reboot → treated as success | `InstallerKind` + `GameInstaller.wineArguments` | — |
